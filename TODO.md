...
| 작업 유형 | 처리 방식 | 예시 |
|-----------|-----------|------|
| 단순 시간/조건 체크 | Node.js (setTimeout, setInterval, DB 쿼리) | 예약 메시지 발송, N시간 침묵 체크 |
| 지능적 판단 필요 | **로컬 LLM (Ollama 등)** | "지금 말 걸어야 하나?", 문서 분류/요약 |
| 사용자 대화 응답 | **설정된 경량 모델** (AIService에서 선택) | 채팅 응답 |

### 토큰 보호 원칙
- ❌ 백그라운드에서 Claude API 절대 호출 금지
- ❌ 주기적 체크에 AI 판단 넣지 않기
- ✅ 단순 비교/쿼리는 순수 코드로
- ✅ AI 판단이 필요한 백그라운드 → 로컬 LLM

### 데이터 영속성 원칙
- ❌ 메모리(Map, 변수)에만 저장 금지 → 서버 재시작 시 날아감
- ✅ 중요 데이터는 반드시 MongoDB 저장
- ✅ 서버 시작 시 DB에서 복구 로직 필수
...
**Phase 1.6.2** (중요): ✅ 완료
- [x] PendingEvent 시스템 (떠남/복귀 추적)
- [x] 예상 vs 실제 비교 로직
- [x] 상황별 aiMemo 생성

**Phase 1.6.3** (선제 메시지): ✅ 완료
- [x] 조건 기반 트리거 시스템
- [x] 스토킹 방지 로직 (쿨다운, 일일/주간 한도, 조용한 시간)
- [x] Socket.io + Notification API 연동
- [x] 인앱 알림 UI

**Phase 1.6.4** (패턴 학습): ✅ 완료
- [x] 사용자 패턴 자동 수집
- [x] 이벤트 소요시간 학습
- [x] 패턴 기반 예측 개선

**Phase 1.6.5** (시간 기반 응답): ✅ 완료
- [x] 시간대별 말투/톤 조절 (새벽엔 조용히, 아침엔 활기차게)
- [x] 오랜만 인사 ("오랜만이다!", "잘 지냈어?")
- [x] 시간 맥락 반영 응답 ("이 시간에 아직 안 잤어?")
- [x] 기념일/특별한 날 인지 (생일, 기념일 등 프로필 연동)

**Phase 1.6.6** (시스템 프롬프트 주입): ✅ 완료
- [x] 현재 시간 정보 주입 (로컬 시각, 요일, 시간대)
- [x] 마지막 대화 시점 정보 ("3일 만에 대화")
- [x] 진행 중인 이벤트 정보 ("여행 중이라고 했음") - PendingEvent
- [x] 사용자 패턴 요약 ("야행성, 주말 활발") - Phase 1.6.4 연동
- [x] 대화 흐름 요약 ("4시간째 같은 주제")

---

## 💬 Phase 1.7: 이벤트 기반 선제 메시지

> 사람처럼 자연스럽게 먼저 말 거는 시스템
> Socket.io 인프라 활용 (Phase 1.6.3에서 구축)

### 핵심 개념
AI가 request-response가 아닌, 필요할 때 먼저 연락하는 구조.
사람이면 당연히 하는 것들:

### 1.7.1 작업 상태 알림
- [ ] 작업 완료: "빌드 끝났어~", "분석 다 했어"
- [ ] 작업 에러: "깃허브 서버 다운이라 커밋 못하는 중"
- [ ] 블로커 발생: "이거 진행하려면 API 키 필요해"
- [ ] 진행률 업데이트: "70% 완료, 예상 5분 남음"

### 1.7.2 MCP 이벤트 감지
- [ ] 홈캠 이상 감지: "달곰이 밥 안 먹은 지 6시간 됐어"
- [ ] 센서 알림: "집 온도 30도 넘었어"
- [ ] 상태 변화: "서버 디스크 90% 찼어"
- [ ] 스케줄 리마인더: "내일 미팅 있어"

### 1.7.3 작업 큐 시스템
- [ ] 백그라운드 작업 큐 (bull/agenda)
...
### 1.7.5 알림 판단 로직
> ⚠️ 단어/패턴 매칭 절대 금지. 맥락 이해 기반 판단.
> 예시는 참고용일 뿐, 규칙으로 하드코딩하면 안 됨.
- [ ] AI가 상황/맥락 보고 "알려줘야 하나?" 스스로 판단
- [ ] 사용자 오버라이드 가능 (이번 건은 안 알려줘도 돼)
- [ ] 이전 대화에서 비슷한 상황 어떻게 했는지 참고

### 구현 방식
```javascript
// 어디서든 호출 가능한 API
proactiveMessenger.sendNow({
  type: 'task_complete',    // 또는 'error', 'blocked', 'mcp_event'
  priority: 'normal',
  title: '빌드 완료',
  message: 'soul-frontend 빌드 끝났어. 확인해볼래?',
  action: { type: 'link', url: '/logs/build' }  // 선택적 액션
});
```
