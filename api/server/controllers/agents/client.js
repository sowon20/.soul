...
class AgentClient extends BaseClient {
  ...
  async buildMessages(
    ...
    const latestText = (latestMessage && (latestMessage.text || latestMessage.content || '')) + '';
    const timeQuery = /시간|몇\s*시|현재\s*시간|지금\s*시간/i;
    if (timeQuery.test(latestText)) {
      const kstDate = new Date();
      const kstDateStr = kstDate.toLocaleDateString('sv-SE', { timeZone: 'Asia/Seoul' });
      const kstTimeStr = kstDate.toLocaleTimeString('sv-SE', { timeZone: 'Asia/Seoul', hour12: false });
      const kstTimestamp = kstDateStr + ' ' + kstTimeStr;
      systemContent = ['If asked about time, answer exactly: 현재 KST: ' + kstTimestamp, systemContent]
        .filter(Boolean)
        .join('\n');
    }
