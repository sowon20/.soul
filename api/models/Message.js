...
async function saveMessage(req, params, metadata) {
  ...
  try {
    ...
    try {
      ...
      const convoId = String(params.conversationId || 'unknown');
      const role = params.isCreatedByUser ? 'user' : 'assistant';
      let text = (params.text ?? '').trim();
      if (!text && Array.isArray(params.content)) {
        text = params.content
      ...
      if (text) {
        const line = JSON.stringify({
          ...
          sender: params.sender ?? null,
                ...
                const filePath = path.join(dir, 'raw.jsonl');
        ...
        try {
          ...
          if (lastLine) {
            ...
            if (parsed && parsed.ts) {
              lastTs = Date.parse(parsed.ts) || 0;
            }
          }
        } catch {}

        // skip if duplicate within 5 seconds (same convo + role + text)
        if (
          lastLine &&
          lastLine.includes(`"conversation_id":"${convoId}"`) &&
          lastLine.includes(`"role":"${role}"`) &&
          lastLine.includes(`"text":"${text.replace(/"/g, '\\"')}"`) &&
          lastTs && nowMs - lastTs < 5000
        ) {
          return;
        }
      ...
      logger.warn(`local memory log failed: ${err?.message || err}`);
    }
